...
def test():
    ...
    if cvar1.get()==1:
        if selone not in randima[0] and seltwo not in randima[0]:
            warn=showwarning(title='Graphic verification code', message ='Verification failed, please verify again!' )
            count=20            
            call()
        else:
            count +=1
    ...
    if cvar5.get()==1 and count!=20:
        if selone not in randima[4] and seltwo not in randima[4]:
            ...
            count +=1
    if count==2 :
        mess=showwarning(title='Graphic verification code', message ='Verified successful, will enter the system!' )
        root.destroy()  
    elif count==0:
        mess=showwarning(title='Graphic verification code', message ='Please select the graphic for further verification!' )
    else:
        defcheck()
            
root.title('Graphic verification code')
root.wm_attributes('-topmost', 1)
root.geometry('500x180')
title=Label(root, text="Please click on the image below").grid(column=0, row=0)
...
cvar4 = IntVar()
cvar5 = IntVar()
check1 = Checkbutton(root, text = "select", variable = cvar1, onvalue = 1, offvalue = 0).grid(column=0, row=2) 
check2= Checkbutton(root, text = "select", variable = cvar2,onvalue = 1, offvalue = 0).grid(column=1, row=2) 
check3 = Checkbutton(root, text = "select", variable = cvar3, onvalue = 1, offvalue = 0).grid(column=2, row=2) 
check4= Checkbutton(root, text = "select", variable = cvar4,onvalue = 1, offvalue = 0).grid(column=3, row=2)
check5 = Checkbutton(root, text = "select", variable = cvar5, onvalue = 1, offvalue = 0).grid(column=4, row=2)
actis = Button(root, text="confirm", command=test,width=8)      
actis.grid(column=3, row=3)    
actcal = Button(root, text="Change to another group", command=call,width=8)      
